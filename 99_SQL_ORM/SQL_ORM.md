...
4. 해당 user 레코드 수정
   ...
   SELECT first_name FROM users_user WHERE id=102;
      ```
...
1. 전체 평균 나이
   ...
   # shell_plus를 사용함면 import 안해도 된다.
...
6. 지역 별 인원수
   ...
   User.objects.values('country').annotate(num_countires=Count('country'))
   User.objects.values('country').annotate(num_countires=Count('country'), avg_balance=Avg('balance'))
   ```
